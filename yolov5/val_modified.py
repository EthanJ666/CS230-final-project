...
def process_batch(detections, labels, iouv):
    ...
    return torch.tensor(correct, dtype=torch.bool, device=iouv.device), iou
...
def run(
    ...
    niou = iouv.numel()
    iou_values_per_class = {i: [] for i in range(nc)}
    # Dataloader
    if not training:
    ...
    s = ("%22s" + "%11s" * 7) % ("Class", "Images", "Instances", "P", "R", "mAP50", "mAP50-95", "Mean_IOU")
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(preds):
            ...
            if nl:
                ...
                # Collect IoUs for each class
                for cls in labels[:, 0].unique():
                    class_mask = labels[:, 0] == cls  # Mask for the current class
                    valid_ious = iou[class_mask].flatten()
                    valid_ious = valid_ious[valid_ious > 0]  # Filter out zero values
                    iou_values_per_class[int(cls.item())].extend(valid_ious.tolist())
        ...
        if plots:
            for si in range(len(paths)):
                plot_images(
                    im[si].unsqueeze(0),  # Select the individual image from the batch
                    output_to_target([preds[si]]),  # Convert prediction to target format for plotting
                    [paths[si]],  # Corresponding image path
                    save_dir / f"{Path(paths[si]).stem}_pred.jpg",  # Save each image separately
                    names
                )
    ...
    mean_ious_per_class = {}
    for cls, ious in iou_values_per_class.items():
        if len(ious) > 0:
            mean_ious_per_class[cls] = np.mean(ious)
        else:
            mean_ious_per_class[cls] = float('nan')

    # Calculate overall mean IoU across all classes
    all_ious = [iou for ious in iou_values_per_class.values() for iou in ious]
    if len(all_ious) > 0:
    ...
    # Print results
    pf = "%22s" + "%11i" * 2 + "%11.3g" * 4 + "%11.3g"  # print format, added mean IoU
    LOGGER.info(pf % ("all", seen, nt.sum(), mp, mr, map50, map, mean_iou_all))
    if nt.sum() == 0:
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            mean_iou_cls = mean_ious_per_class.get(c, float('nan'))
            LOGGER.info(pf % (names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i], mean_iou_cls))
